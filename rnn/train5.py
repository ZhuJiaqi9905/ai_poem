...
def train(model,loss_func,optim,poem_vec_lst5, num_epochs, model_path):
     ...
     for epoch in range(start_epoch,start_epoch+num_epochs):
          ...
          for idx in range(len(poem_vec_lst5)//BATCH_SIZE):
               print(idx*BATCH_SIZE,"/",len(poem_vec_lst5))
               batch_x,batch_y=get_batch(poem_vec_lst5,BATCH_SIZE,idx, 5)#得到batch_x,batch_y
          ...
          torch.save(model.state_dict(), model_path + "\\rnn5_epoch_%d.pth"%(epoch))
...
sentence_len=5  #五言
net5=Net(sentence_len=sentence_len,batch_size=BATCH_SIZE,vocab_size=vocab_size,embed_size=embed_size,hidden_size=hidden_size)
loss_function=torch.nn.CrossEntropyLoss()
optimizer=torch.optim.Adam(net5.parameters(),lr = LR)
...
net5.embedding.weight.data.copy_(word_vec_net[0].weight.data)

#训练
model_path = root_path + '\\Models\\rnn'
train(net5,loss_function,optimizer,poem_vec_lst5, 10, model_path)
