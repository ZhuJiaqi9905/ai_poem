...
# %%
def trainIters(encoder, decoder, wd2Idx, epoch, num_topic, model_path, print_every=100, plot_every=100, learning_rate=0.005):
    ...
    for ep in range(epoch):
        ...
        torch.save(encoder.state_dict(), model_path + "\\seq2seq_encoder_%d.pth"%(ep) )
        torch.save(decoder.state_dict(), model_path + "\\seq2seq_decoder_%d.pth"%(ep) )
...
word_vec_path = root_path + "\\Data\\word_vector_for_seq.pth"
word_vec_net = nn.Sequential(nn.Embedding(len(vocab), 200),
                         nn.Embedding(len(vocab), 200))
word_vec_net.load_state_dict(torch.load( word_vec_path,map_location=torch.device('cpu')))
...
encoder.load_state_dict(torch.load(model_path + '\\seq2seq_encoder.pth', map_location='cuda:0'))
decoder.load_state_dict(torch.load(model_path + '\\seq2seq_decoder.pth', map_location='cuda:0'))
encoder = encoder.to(device)
decoder = decoder.to(device)


trainIters(encoder, decoder,wd2Idx,epoch=10, num_topic=num_topic,print_every=5,learning_rate=0.001, model_path=model_path)
