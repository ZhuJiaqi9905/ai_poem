...
class Sample(data.Dataset):
    ...
    采样类：用于对原始数据进行采样
    '''
    def __init__(self, vocab ,idx2word, word2idx,  poem_vec_lst5, poem_vec_lst7, max_window_size, noise_num):
        '''
        raw_data_set 最初的数据集
        ...
        self.vocab = vocab
        self.data_set = poem_vec_lst5[:] + poem_vec_lst7[:]
        self.words_num = sum([len(poem) for poem in self.data_set])
        #counter是一个dict,对应每个word的索引和它的出现次数
        self.counter = collections.Counter([word for poem in self.data_set for word in poem])    
    ...
    def get_negative_samples(self):
        ...
        sampling_weights = [self.counter[word]**0.75 for word in self.idx2word.keys()]
